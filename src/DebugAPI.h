...
namespace DrawDebug
{
	...
	template <glm::vec4 Color = Colors::RED, int time = 3000, float height = 20.0f>
	void draw_line(const RE::NiPoint3& __from, int part, float size = 5.0f)
	{
		RE::NiPoint3 _from = __from + RE::NiPoint3{ 0, 0, height } * part;
		RE::NiPoint3 _to = _from + RE::NiPoint3{ 0, 0, height };
		glm::vec3 from(_from.x, _from.y, _from.z);
		glm::vec3 to(_to.x, _to.y, _to.z);
		DebugAPI::DrawLineForMS(from, to, time, Color, size);
	}
